// The system prompt is assembled from named sections so the hybrid engine can
// reuse everything except the decision order. SYSTEM_PROMPT must stay byte
// identical to the pre-split text; tests/prompt.test.ts checks it against a
// verbatim snapshot. Edit the section constants, never the join.

const INTRO = `You are a triage classifier for bug reports submitted to a legal-AI SaaS product (project workspaces, document upload, chat-based AI review, billing, auth). For every report you receive, you must call the classify_bug_report tool exactly once with a structured output. Do not reply in prose.`;

const YOUR_JOB = `# Your job
...
const CLASSIFICATIONS = `# Classifications — pick exactly one

${CLASSIFICATION_DEFINITIONS}`;

const DECISION_ORDER = `# Decision order
...
const SEVERITY = `# Severity rubric
...
const CATEGORY = `# Category — pick the closest
...
const ANTI_HALLUCINATION = `# Anti-hallucination rules
...
const FIELD_DISCIPLINE = `# CRITICAL: field discipline by classification
...
// ---------------------------------------------------------------------------
// Drafting prompt for --engine jev.
//
// The bucket has already been chosen by Jev before this prompt is sent, so the
// decision order is gone and the job is reduced to filling in the fields. The
// bucket is also pinned in the tool schema (see toolInputSchemaFor), so this
// prompt is the polite request and the schema is the contract.
// ---------------------------------------------------------------------------

const DRAFT_INTRO = `You are drafting the structured triage record for a bug report submitted to a legal-AI SaaS product (project workspaces, document upload, chat-based AI review, billing, auth). The classification has already been decided by a separate triage layer and is stated in the user message. Do not change it. For every report you receive, you must call the classify_bug_report tool exactly once with a structured output. Do not reply in prose.`;

const DRAFT_YOUR_JOB = `# Your job

Populate the fields for the given classification. Do not invent details that aren't present or strongly implied by the user's message.`;

const DRAFT_CLASSIFICATIONS = `# Classification definitions (for reference; yours is already chosen)
...
export const DRAFT_SYSTEM_BLOCKS = [
  {
    type: 'text' as const,
